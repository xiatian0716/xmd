...
func (t *DefaultTemplate) Execute(data interface{}) (*bytes.Buffer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(`template parse error:%s,%s`, t.text, err)
	}

	buf := new(bytes.Buffer)
	if err = tem.Execute(buf, data); err != nil {
		return nil, fmt.Errorf(`template execute error:%s,%s`, t.text, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf(`go format error:%s,%s`, buf.String(), err)
	}
